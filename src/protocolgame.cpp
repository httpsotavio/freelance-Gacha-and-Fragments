...
void ProtocolGame::login(const std::string& name, uint32_t accountId, OperatingSystem_t operatingSystem)
{
	...
	if (OTCv8 || operatingSystem >= CLIENTOS_OTCLIENT_LINUX) {
		if (OTCv8) {
			sendFeatures();
		}
...
void ProtocolGame::onRecvFirstMessage(NetworkMessage& msg)
{
	...
	if (OTCv8StringLength == 5 && msg.getString(5) == "OTCv8") {
		OTCv8 = msg.get<uint16_t>() != 0; // 253, 260, 261, ...
	}
...
void ProtocolGame::MoveUpCreature(NetworkMessage& msg, const Creature* creature, const Position& newPos, const Position& oldPos)
{
	...
	if (newPos.z == 7) {
		if (OTCv8) {
			...
			for (int32_t z = 5; z >= 0; --z) {
				GetFloorDescription(msg, oldPos.x - awareRange.left(), oldPos.y - awareRange.top(), z, awareRange.horizontal(), awareRange.vertical(), 8 - z, skip);
			}
	...
	if (newPos.z == 8) {
		if (OTCv8) {
			...
			for (int32_t offset = 1; offset <= 3; ++offset) {
				GetFloorDescription(msg, oldPos.x - awareRange.left(), oldPos.y - awareRange.top(), newPos.z + offset - 1, awareRange.horizontal(), awareRange.vertical(), -offset, skip);
			}
