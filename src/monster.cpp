	...
	mType(mtype)
{
	...
	// auto register creature events
	for (const auto& it : g_creatureEvents->getEvents()) {
		AutoRegister_t autoRegister = it.second->getAutoRegister();
		if (autoRegister == AUTOREGISTER_ALL || autoRegister == AUTOREGISTER_MONSTERS) {
			registerCreatureEvent(it.second->getName());
		}
	}
}
	...
	if (!isInSpawnRange(position)) {
		...
		if (!isIdle) {
			...
			if (isSummon()) {
				...
				if (attackedCreature && attackedCreature->isInGhostMode()) {
					removeTarget(attackedCreature);
					removeFriend(attackedCreature);
