...
bool IOLoginData::loadPlayer(Player* player, DBResult_ptr result)
{
	...
	//load string storage map
	query.str(std::string());
	query << "SELECT `key`, `value` FROM `player_stringstorage` WHERE `player_id` = " << player->getGUID();
	if ((result = db->storeQuery(query.str()))) {
		do {
			player->setStringStorageValue(result->getString("key"), result->getString("value"));
		} while (result->next());
	}
	...
	player->changeVocation(player->getVocation()->getId());
	return true;
}
...
bool IOLoginData::savePlayer(Player* player)
{
	...
	// save string storage
	query.str(std::string());
	query << "DELETE FROM `player_stringstorage` WHERE `player_id` = " << player->getGUID();
	if (!db->executeQuery(query.str())) {
		return false;
	}
	...
	DBInsert strStorageQuery("INSERT INTO `player_stringstorage` (`player_id`, `key`, `value`) VALUES ");
	for (const auto& it : player->stringStorageMap) {
		query << player->getGUID() << ',' << db->escapeString(it.first) << ',' << db->escapeString(it.second);
		if (!strStorageQuery.addRow(query)) {
			return false;
		}
	}
