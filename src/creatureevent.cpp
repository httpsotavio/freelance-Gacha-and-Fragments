...
bool CreatureEvent::configureEvent(const pugi::xml_node& node)
{
	...
	if (tmpStr == "login") {
		...
		type = CREATURE_EVENT_EQUIP;
	} else if (tmpStr == "unequip") {
		type = CREATURE_EVENT_UNEQUIP;
	} else {
		std::cout << "[Error - CreatureEvent::configureEvent] Invalid type for creature event: " << eventName << std::endl;
		return false;
	}

	if (pugi::xml_attribute attribute = node.attribute("autoregister")) {
		tmpStr = asLowerCaseString(attribute.as_string());
		if (tmpStr == "player" || tmpStr == "players") {
			autoRegister = AUTOREGISTER_PLAYERS;
		} else if (tmpStr == "monster" || tmpStr == "monsters") {
			autoRegister = AUTOREGISTER_MONSTERS;
		} else if (tmpStr == "all" || tmpStr == "everyone") {
			autoRegister = AUTOREGISTER_ALL;
		} else if (tmpStr == "none") {
			autoRegister = AUTOREGISTER_NONE;
		} else {
			std::cout << "[Warning - CreatureEvent::configureEvent] Invalid autoregister for creature event: " << eventName << std::endl;
		}
	}
...
std::string CreatureEvent::getScriptEventName() const
{
	...
	switch (type) {
		...
		case CREATURE_EVENT_EQUIP:
			return "onEquip";

		case CREATURE_EVENT_UNEQUIP:
			return "onUnequip";
...
void CreatureEvent::executeHealthChange(Creature* creature, Creature* attacker, CombatDamage& damage, bool inherited)
{
	//onHealthChange(creature, attacker, primaryDamage, primaryType, secondaryDamage, secondaryType, origin, inherited)
	if (!scriptInterface->reserveScriptEnv()) {
	...
	LuaScriptInterface::pushBoolean(L, inherited);
...
void CreatureEvent::executeManaChange(Creature* creature, Creature* attacker,
	int32_t& manaChange, CombatOrigin origin, bool inherited)
{
	//onManaChange(creature, attacker, manaChange, origin, inherited)
	if (!scriptInterface->reserveScriptEnv()) {
...
void CreatureEvent::executeEquip(Player* player, uint8_t slot, Item* item, bool isLogInOut)
{
	//onEquip(player, item, slot, isLogin)
	//onUnequip(player, item, slot, isLogout)
	if (!scriptInterface->reserveScriptEnv()) {
		std::cout << "[Error - CreatureEvent::executeEquip] Call stack overflow" << std::endl;
		return;
	}
	...
	LuaScriptInterface::pushBoolean(L, isLogInOut);
