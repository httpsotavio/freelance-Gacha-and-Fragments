...
uint32_t MoveEvents::onPlayerEquip(Player* player, Item* item, slots_t slot,
	bool isCheck, bool isLogin/* = false*/)
{
	MoveEvent* moveEvent = getEvent(item, MOVE_EVENT_EQUIP, slot);
	if (!moveEvent) {
		return 1;
	}
	return moveEvent->fireEquip(player, item, slot, isCheck, isLogin);
}

uint32_t MoveEvents::onPlayerDeEquip(Player* player, Item* item, slots_t slot, bool isLogout)
{
	MoveEvent* moveEvent = getEvent(item, MOVE_EVENT_DEEQUIP, slot);
	if (!moveEvent) {
		return 1;
	}
	return moveEvent->fireEquip(player, item, slot, true, isLogout);
}
...
uint32_t MoveEvent::EquipItem(MoveEvent* moveEvent, Player* player, Item* item, slots_t slot,
	bool isCheck, bool isLogin)
{
	...
	for (CreatureEvent* creatureEvent : player->getCreatureEvents(CREATURE_EVENT_EQUIP)) {
		creatureEvent->executeEquip(player, slot, item, isLogin);
	}
...
uint32_t MoveEvent::DeEquipItem(MoveEvent*, Player* player, Item* item, slots_t slot, bool, bool isLogout)
{
	if (!player->isItemAbilityEnabled(slot)) {
		return 1;
	}
	...
	for (CreatureEvent* creatureEvent : player->getCreatureEvents(CREATURE_EVENT_UNEQUIP)) {
		creatureEvent->executeEquip(player, slot, item, isLogout);
	}
...
uint32_t MoveEvent::fireEquip(Player* player, Item* item, slots_t slot, bool boolean, bool isLogInOut)
{
	if (scripted) {
		return executeEquip(player, item, slot, isLogInOut);
	} else {
		return equipFunction(this, player, item, slot, boolean, isLogInOut);
	}
}

bool MoveEvent::executeEquip(Player* player, Item* item, slots_t slot, bool isLogInOut)
{
	//onEquip(player, item, slot, isLogin)
	//onDeEquip(player, item, slot, isLogout)
	if (!scriptInterface->reserveScriptEnv()) {
