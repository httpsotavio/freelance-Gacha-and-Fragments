...
bool Game::placeCreature(Creature* creature, const Position& pos, bool extendedPos /*=false*/, bool forced /*= false*/)
{
	...
	if (creature->getPlayer()) {
		for (const auto& it : g_creatureEvents->getEvents()) {
			const CreatureEventType_t eventType = it.second->getEventType();
			if (eventType != CREATURE_EVENT_LOGIN && eventType != CREATURE_EVENT_LOGOUT) {
				const AutoRegister_t autoRegister = it.second->getAutoRegister();
				if (autoRegister == AUTOREGISTER_ALL || autoRegister == AUTOREGISTER_PLAYERS) {
					creature->registerCreatureEvent(it.second->getName());
				}
			}
		}
	}
...
{
	if (!creature->getCanMove() && !hasBitSet(FLAG_NOLIMIT, flags)) {
...
void Game::playerSetAttackedCreature(uint32_t playerId, uint32_t creatureId)
{
	...
	if (attackCreature->getMonster()) {
		const Monster* monster = attackCreature->getMonster();
		int32_t saga = -1;
		player->getStorageValue(PSTRG_SAGA, saga);
		if (saga != monster->getSaga() && monster->getSaga() != -1) {
			std::ostringstream ss;
...
void Game::checkCreatures(size_t index)
{
	...
	for (size_t i = 0; i < EVENT_CREATURECOUNT; ++i) {
		auto& checkCreatureList = checkCreatureLists[i];
		auto it = checkCreatureList.begin(), end = checkCreatureList.end();
		while (it != end) {
			Creature* creature = *it;
			if (creature->creatureCheck) {
				if (creature->getHealth() > 0) {
					creature->onAttacking(EVENT_CHECK_CREATURE_INTERVAL);
					if (index == i) {
...
bool Game::combatChangeHealth(Creature* attacker, Creature* target, CombatDamage& damage, bool isEvent,
	int32_t* damageDealt/* = nullptr*/)
{
	const Position& targetPos = target->getPosition();
	if (damage.primary.value > 0 && damage.primary.type == COMBAT_HEALING) {
		...
		if (damage.origin != ORIGIN_NONE) {
			...
			if (!events.empty()) {
				...
				return combatChangeHealth(attacker, target, damage, true, damageDealt);
			}
		}
		...
		if (target->hasCondition(CONDITION_MANASHIELD) && damage.primary.type != COMBAT_UNDEFINEDDAMAGE) {
			...
			if (manaDamage != 0) {
				int32_t prevHealthChange = healthChange;
				if (damage.origin != ORIGIN_NONE) {
					const auto& events = target->getCreatureEvents(CREATURE_EVENT_MANACHANGE);
					if (!events.empty()) {
						for (CreatureEvent* creatureEvent : events) {
							creatureEvent->executeManaChange(target, attacker, healthChange, damage.origin, true);
						}
				...
				if (healthChange > manaDamage) {
					damage.primary.value += (prevHealthChange - healthChange);
				}
		...
		if (damage.origin != ORIGIN_NONE) {
			...
			if (!events.empty()) {
				for (CreatureEvent* creatureEvent : events) {
					creatureEvent->executeHealthChange(target, attacker, damage, inherited);
				}
...
bool Game::combatChangeMana(Creature* attacker, Creature* target, int32_t manaChange, CombatOrigin origin)
{
	if (manaChange > 0) {
		...
		if (origin != ORIGIN_NONE) {
			...
			if (!events.empty()) {
				for (CreatureEvent* creatureEvent : events) {
					creatureEvent->executeManaChange(target, attacker, manaChange, origin, false);
				}
