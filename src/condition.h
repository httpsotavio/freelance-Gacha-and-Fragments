...
class ConditionSuppress final : public Condition
{
public:
	ConditionSuppress(ConditionId_t id, ConditionType_t type, int32_t ticks,
		bool buff, uint32_t subId) :
		Condition(id, type, ticks, buff, subId) {}
	...
	ConditionSuppress* clone() const final {
		return new ConditionSuppress(*this);
	}

	bool addConditionType(ConditionType_t conditionType) {
		return conditionTypes.insert(conditionType).second;
	}

protected:
	std::unordered_set<ConditionType_t> conditionTypes;
};
