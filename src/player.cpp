...
void Player::onCreatureAppear(Creature* creature, bool isLogin)
{
	...
	if (isLogin && creature == this) {
		...
		for (int32_t slot = CONST_SLOT_FIRST; slot <= CONST_SLOT_LAST; ++slot) {
			...
			if (item) {
				item->startDecaying();
				g_moveEvents->onPlayerEquip(this, item, static_cast<slots_t>(slot), false, true);
			}
		}
...
void Player::postRemoveNotification(Thing* thing, const Cylinder* newParent, int32_t index, cylinderlink_t link /*= LINK_OWNER*/)
{
	if (link == LINK_OWNER) {
		//calling movement scripts
		g_moveEvents->onPlayerDeEquip(this, thing->getItem(), static_cast<slots_t>(index), index == 0);
	}
...
bool Player::canWear(uint32_t lookType, uint8_t addons) const
{
	...
	if (!outfit->vocations.empty()) {
		bool hasVocation = false;

		int32_t myVocId = static_cast<int32_t>(getVocationId());
		for (int32_t vocId : outfit->vocations) {
			if (vocId == myVocId) {
				hasVocation = true;
				break;
			}
