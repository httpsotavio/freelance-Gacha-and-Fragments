...
class Creature : virtual public Thing
{
	...
	public:
		...
		virtual void addStorageValue(const uint32_t key, const int32_t value, const bool = false);
		bool getStorageValue(const uint32_t key, int32_t& value) const;

		void setStringStorageValue(const std::string& key, const std::string& value);
	...
	protected:
		...
		std::map<std::string, std::string> stringStorageMap;
