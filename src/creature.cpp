...
void Creature::onWalk()
{
	...
	if (cancelNextWalk || isMovementBlocked()) {
		listWalkDir.clear();
		onWalkAborted();
...
void Creature::startAutoWalk()
{
	...
	size_t size = 0;
	for (auto it = listWalkDir.begin(); it != listWalkDir.end() && size <= 1; ++it) {
		size++;
	}
	addEventWalk(size == 1);
}
...
void Creature::addStorageValue(const uint32_t key, const int32_t value, const bool)
{
	if (value != -1) {
...
bool Creature::getStorageValue(const uint32_t key, int32_t& value) const
{
...
void Creature::setStringStorageValue(const std::string& key, const std::string& value)
{
	if (!value.empty()) {
...
bool Creature::getStringStorageValue(const std::string& key, std::string& value) const
{
