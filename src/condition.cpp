...
Condition* Condition::createCondition(ConditionId_t id, ConditionType_t type, int32_t ticks, int32_t param/* = 0*/, bool buff/* = false*/, uint32_t subId/* = 0*/)
{
	switch (type) {
		...
		case CONDITION_SUPPRESS:
			return new ConditionSuppress(id, type, ticks, buff, subId);
...
bool ConditionSuppress::startCondition(Creature* creature)
{
	if (!Condition::startCondition(creature)) {
		return false;
	}
	...
	if (Player* player = creature->getPlayer()) {
		for (auto condType : conditionTypes) {
			player->addConditionSuppressions(static_cast<uint32_t>(condType));
			if (condType == CONDITION_PARALYZE || condType == CONDITION_PARALYZE) {
				player->removeCondition(condType, true);
			}
		}
...
void ConditionSuppress::endCondition(Creature* creature)
{
	if (Player* player = creature->getPlayer()) {
		for (auto condType : conditionTypes) {
			player->removeConditionSuppressions(static_cast<uint32_t>(condType));
		}
...
void ConditionSuppress::addCondition(Creature* creature, const Condition* addCondition)
{
	if (!updateCondition(addCondition)) {
		return;
	}
	...
	const ConditionSuppress& conditionSuppress = static_cast<const ConditionSuppress&>(*addCondition);
	conditionTypes = conditionSuppress.conditionTypes;
