...
ReturnValue Combat::canDoCombat(Creature* caster, Tile* tile, bool aggressive, uint16_t)
{
...
bool Combat::setCallback(CallBackParam_t key)
{
	switch (key) {
		...
		case CALLBACK_PARAM_DAMAGETARGET: {
			params.damageCallback.reset(new DamageCallback());
			return true;
		}
	}
	return false;
...
CallBack* Combat::getCallback(CallBackParam_t key)
{
	switch (key) {
		...
		case CALLBACK_PARAM_DAMAGETARGET: {
			return params.damageCallback.get();
		}
	}
	return nullptr;
}
...
void Combat::CombatHealthFunc(Creature* caster, Creature* target, const CombatParams& params, CombatDamage* data)
{
	...
	if (g_game.combatChangeHealth(caster, target, damage, false, &damageDealt)) {
		CombatConditionFunc(caster, target, params, nullptr);
		CombatDispelFunc(caster, target, params, nullptr);
	}

	if (params.damageCallback) {
		params.damageCallback->onDamageTarget(caster, target, damageDealt);
	}
}
...
void ValueCallback::getMinMaxValues(Player* player, CombatDamage& damage, bool useCharges) const
{
	...
	if ((lua_gettop(L) + parameters + 1) != size) {
...
void DamageCallback::onDamageTarget(Creature* creature, Creature* target, int32_t damage) const
{
	//onDamageTarget(creature, target, damage)
	if (!scriptInterface->reserveScriptEnv()) {
		std::cout << "[Error - DamageCallback::onDamageTarget] Call stack overflow" << std::endl;
		return;
	}
